...
from utils.types import hidden_dim, activation, frac_4
from utils.math_ops import is_bin_times
multi_class = dict(hidden_dim = hidden_dim,
                   activation = activation,
                   drop_out   = frac_4)

from models.backend import leaves_config, stem_config
from models.backend import Leaves, Stem
penn_tree_config = dict(embed_layer = leaves_config,
...
class PennTree(nn.Module):
    def __init__(self,
        ...
        self._emb_layer = Leaves(num_words, initial_weights, **embed_layer)
        unit_dim = self._emb_layer.embedding_dim
        ...
        activation = {'sigmoid': nn.Sigmoid, 'tanh':nn.Tanh, 'relu': nn.ReLU}[activation]
    ...
    def forward(self,
        ...
        static, dynamic = self._emb_layer(is_training, word_idx)
        if dynamic is None:
            base_inputs = static
        else:
            base_inputs = dynamic
        if self._paddings:
            bottom_existence = torch.ones(batch_size, batch_len, dtype = torch.bool)
        else:
            bottom_existence = word_idx > 0

        (layers_of_base,
         layers_of_orient,
         layers_of_existence) = self._stem_layer(bottom_existence,
        ...
        return batch_size, batch_len, static, dynamic, layers_of_base, layers_of_existence, layers_of_orient, tags, labels
...
class PennOperator(Operator):
    ...
    def _step(self, mode, ds_name, batch, flush = True, extra = None):
        ...
        orient_info = gold_orients if mode == M_TRAIN else None #(batch['offset'], batch['length'])
        (batch_size, batch_len, static, dynamic, layers_of_base, existences, orient_logits, tag_logits,
         label_logits) = self._model(batch['word'], orient_info)
        ...
        if mode == M_TRAIN:
            ...
            if vis.save_tensors:
                ...
                if self._orient_hinge_loss: # otherwise with sigmoid
                    ...
                    orient_logits += 1
                    orient_logits /= 2
                b_mpcs = (mpc_word.type(torch.float16), mpc_label.type(torch.float16))
            ...
            vis._process(batch_id, b_size + b_head + b_data)
...
def cross_entropy(x_, y_, w_, *s_l_):
    ...
    if s_l_:
        s_, l_ = s_l_
        p_ = torch.arange(t_, device = l_.device)[None, :, None]
        t_ = s_index(s_ - l_)[:, None, None]
        losses = losses * (p_ >= t_).view(-1)
