...
p <- p + scale_x_continuous(limits = c(-0.8, 1.1), breaks = c(-0.7, -0.3, 0, 0.3, 0.7), labels = c('Max.Tag', -0.3, 0, 0.3, 'Max.Label'), expand = c(0, 0))
...
p <- p + theme(legend.position = c(0.89, 0.67),
            ...
            #    text = element_text(size = 10),
               ...
               plot.tag = element_text(size = 9.45, color = "#4D4D4D"))
p <- p + labs(tag = "Max.Orientation          (0.2, 0.3, 0.5)")
p <- p + geom_segment(
                x = 0.37, y = 0.75,
                xend = 0.1, yend = 0.5,
                ...
                colour = "#4D4D4D" # Also accepts "red", "blue' etc
  )
